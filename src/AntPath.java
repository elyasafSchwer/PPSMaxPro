...
public class AntPath extends Path{
	...
	public AntPath(Path last_list, CityAnt city_ant, int next_step, double money) {
		super(last_list, next_step, money);
		this.h_v_p = 0;
		for(int i = last_list.getVertexList().size() - 1; i < getVertexList().size() - 1; i++){
			this.h_v_p += city_ant.getAntOf(getGraph().getEdge(getVertexList().get(i), getVertexList().get(i + 1)));
		}
